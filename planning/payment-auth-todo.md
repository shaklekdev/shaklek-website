...
- [x] **Webhook handling** — done and verified end-to-end 2026-08-14. `website/src/app/api/webhooks/stripe/route.ts` verifies the signature, marks the order `paid` on `checkout.session.completed`, and sends the stylist notification email at that point (not earlier). Confirmed live in production: real test-mode Stripe account, endpoint registered via Stripe's "Event destinations" UI (scope: Your account, payload style: Snapshot), `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` set in Amplify, full checkout run with Stripe's `4242...` test card — order correctly flipped `pending_payment` → `paid` in Neon. Still test mode only; going live needs the Wio-backed Stripe merchant account approved (see `incorporation-todo.md`).
