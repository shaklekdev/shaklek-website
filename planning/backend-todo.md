...
- [x] **Tailor spec sheet + WhatsApp handoff — built 2026-08-17, human-in-the-loop, not automated.** Founder descoped this from full automation once the real WhatsApp Business Platform requirement (Meta business verification, message template approval) came up as a genuine external blocker, same category as the Wio bank account. Shipped instead: `GET /api/dashboard/orders/[id]/spec-sheet` (staff-only, `PDFDocument` via `pdfkit`) generates a per-order PDF with fabric/color/size/measurements/customization from `order_items`; `/dashboard/orders` has a "Spec sheet" link (opens/downloads it) and a "Send to tailor" link (`wa.me/<TAILOR_WHATSAPP_NUMBER>`, prefilled with a short order summary) next to it. Staff downloads the PDF, clicks "Send to tailor" to open the chat, attaches the PDF themselves, sends it. `TAILOR_WHATSAPP_NUMBER` env var not set yet — "Send to tailor" grays out until it is (same pattern as `getDb()` returning null when `DATABASE_URL` is unset). Needed `serverExternalPackages: ["pdfkit"]` in `next.config.ts` — pdfkit reads its font `.afm` files relative to its own package dir at runtime, bundling it breaks that lookup (`ENOENT` on `Helvetica.afm`), a known pdfkit + Next.js App Router issue.
