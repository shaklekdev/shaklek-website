...
This is the post no competitor can copy, because it needs the imagery of a
...
> the image changes with you, so you see the piece you are actually
...
>    image change with it.
> 2. Measure. Pick XS to XXL, or send us your own numbers and it's cut to those.
...
| `t-s2.png` | 100% LINEN / NOTHING ELSE | ⚠️ Tile art still reads COTTON AND LINEN and must be re-rendered: cotton has no supplier and is switched off in fabrics.ts. Linen breathes in Gulf heat and softens with every wash. |
| `t-s3.png` | XS to XXL, OR YOUR OWN MEASUREMENTS / SAME PRICE | A size chart is an average of everyone, which makes it exact for no one. |
...
Placed at grid positions 2, 4, 6 and 8 they read: **CUT, FOR YOUR BODY, NOT, A SIZE CHART.** Post 9 → 1 in reverse so the grid assembles correctly.
...
| XS to XXL or own measurements, same price | `website/src/app/how-it-works/page.tsx:38` |
...
- Posts 1/3/5/9 of the original launch grid still need real photography that does not
